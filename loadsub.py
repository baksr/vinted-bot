    ...
    for weburl in configs["suburl"]:
        if configs["suburl"][weburl]["salon"] == ctx.channel.name:
            with open("config.json", 'w+') as configedit:
                ...
                json.dump(configs, configedit, indent=4)
            await ctx.send(
                f"{ctx.author.mention} - **✔️ Le lien du scrapping lié au salon {ctx.channel.name} a été modifié avec succès !**")
            return
    ...
    for weburl in configs["suburl"]:
        if configs["suburl"][weburl]["salon"] == ctx.channel.name:
            ...
            with open("config.json", 'w+') as configedit:
                ...
                json.dump(configs, configedit, indent=4)
            await ctx.send(
                f"{ctx.author.mention} - **✔️ Le lien du scrapping lié au salon {ctx.channel.name} a été supprimé avec succès !**")
            return
...
async def help(ctx):
    ...
    embed.add_field(name="**change_url**", value=f"**Usage :** ``{configs['prefix']}change_url nouvel_url_vinted``",
                    inline=False)
    embed.add_field(name="**change_color_text**",
                    value=f"**Usage :** ``{configs['prefix']}change_color_text (YAML/fix)``", inline=False)
    embed.add_field(name="**change_color_embed**",
                    value=f"**Usage :** ``{configs['prefix']}change_color_embed couleur``", inline=False)
